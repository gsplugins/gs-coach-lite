jQuery(function($) {
	...
	function load_gs_coach_ticker_class() {
	
		function GS_Coach_Ticker( $widget ) {
	
			this.$widget = $widget;
			this.$widgetContainer = $widget.closest('.gs_coach_area');
		...
		GS_Coach_Ticker.prototype.getOptions = function() {
	
			return this.$widget.data( 'carousel-config' ) || {};
	
		}
	
		GS_Coach_Ticker.prototype.getDefaults = function() {
		...
		GS_Coach_Ticker.prototype.setup = function() {
		...
		GS_Coach_Ticker.prototype.breakpointsSetup = function() {

			var mobileLogos = this.config.mobileLogos;
			var desktopLogos = this.config.desktopLogos;
			var tabletLogos = this.config.tabletLogos;
			var slidesPerGroup = this.config.slidesPerGroup;

			this.config.breakpoints = {
				0: {
					slidesPerView: mobileLogos,
					slidesPerGroup: slidesPerGroup > mobileLogos ? mobileLogos : slidesPerGroup
				},
				768: {
					slidesPerView: tabletLogos,
					slidesPerGroup: slidesPerGroup > tabletLogos ? tabletLogos : slidesPerGroup
				},
				1024: {
					slidesPerView: desktopLogos,
					slidesPerGroup: slidesPerGroup > desktopLogos ? desktopLogos : slidesPerGroup
				}
			}

		}

		GS_Coach_Ticker.prototype.getMaxLogos = function() {
			if ( window.matchMedia("(min-width: 1025px)").matches ) return this.config.desktopLogos;
			if ( window.matchMedia("(min-width: 768px)").matches ) return this.config.tabletLogos;
			return this.config.mobileLogos;
		}

		GS_Coach_Ticker.prototype.fixItemWidth = function() {
			var baseAreaWidth = this.$widgetContainer.width();
			var maxLogos = this.getMaxLogos();
			var singleItemWidth = baseAreaWidth / maxLogos;
			this.$widget.find('.gs_coach_single--wrapper').css('width', singleItemWidth + 'px');
		}

		GS_Coach_Ticker.prototype.run = function() {

			this.fixItemWidth();

			$(window).on( 'resize', gs_coach_debounce(function() {
				this.fixItemWidth();
			}.bind(this)));
		...
		$.fn.gs_coach_ticker = function() {
			return new GS_Coach_Ticker( $(this).first() );
		}
	...
	function gs_coach_do_ticker( $tickers ) {

		if ( ! $tickers.length ) return;

		// Load ticker class if not loaded
		if ( ! ('gs_coach_ticker' in $.fn) ) load_gs_coach_ticker_class();

		$tickers.each(function() {
			$(this).gs_coach_ticker();
		});

		// $tickers.each(function() {
		// 	var size = $(this).parent().width();
		// 	console.log( $(this).data('carousel-config') )
		// });

		// $tickers.carouselTicker({
		// 	direction: "prev"
		// });
