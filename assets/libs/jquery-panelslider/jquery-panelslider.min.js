...
(function ($) {
    ...
    function _slideIn(panel, options) {
        ...
        $("#gscoach-overlay").bind("touchmove", false);
        $("#gscoach-overlay").fadeIn(200);
    ...
    $.panelslider.next = function () {
        var active = $('.gs-active-panel');
        var to = active.next('.gscoach-panel');
        if ( ! to.length ) to = active.parent().find('.gscoach-panel').first();
        _slideTo(active, to, active.data());
    };

    $.panelslider.prev = function () {
        var active = $('.gs-active-panel');
        var to = active.prev('.gscoach-panel');
        if ( ! to.length ) to = active.parent().find('.gscoach-panel').last();
        _slideTo(active, to, active.data());
    };
