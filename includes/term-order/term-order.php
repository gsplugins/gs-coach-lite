...
final class Term_Order {
	...
	public function init() {
		 ...
		 */
		$this->db_strategy = apply_filters( 'gs_coach_db_strategy', $this->db_strategy );
		...
		foreach ( $this->taxonomies as $value ) {
			...
			register_term_meta( $value, 'order', array(
				'type'              => 'integer',
				'description'       => esc_html__( 'Numeric order for terms, useful when sorting', 'gscoach' ),
				'default'           => 0,
				'single'            => true,
				'show_in_rest'      => true,
			) );
		...
		// Ajax actions
		add_action( 'wp_ajax_gs_coach_reordering_terms', array( $this, 'ajax_reordering_terms' ) );
	...
	public function taxonomy_supported( $taxonomy = array() ) {
		...
		// Filter & return
		return (bool) apply_filters( 'gs_coach_taxonomy_supported', $retval, $taxonomy );
	}
	...
	public function taxonomy_override_orderby_supported( $taxonomy = array() ) {
		...
		// Filter & return
		return (bool) apply_filters( 'gs_coach_taxonomy_override_orderby_supported', $retval, $taxonomy );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'gs-coach-term-order-quick-edit', $this->url . 'includes/term-order/assets/js/quick-edit.js', array( 'jquery' ), $this->db_version, true );

		// Enqueue fancy ordering
		if ( true === $this->fancy ) {
			wp_enqueue_script( 'gs-coach-term-order-reorder', $this->url . 'includes/term-order/assets/js/reorder.js', array( 'jquery-ui-sortable' ), $this->db_version, true );
		}
	}
	...
	public function help_tabs() {
		...
		if ( true === $this->fancy ) {
			get_current_screen()->add_help_tab( array(
				'id'      => 'gscoach_term_order_help_tab',
				'title'   => esc_html__( 'Term Order', 'gscoach' ),
				'content' => '<p>' . esc_html__( 'To reposition an item, drag and drop the row by "clicking and holding" it anywhere and moving it to its new position.', 'gscoach' ) . '</p>',
			) );
		...
		// Numbers only
			...
			get_current_screen()->add_help_tab( array(
				'id'      => 'gscoach_term_order_help_tab',
				'title'   => esc_html__( 'Term Order', 'gscoach' ),
				'content' => '<p>' . esc_html__( 'To position an item, Quick Edit the row and change the order value to a more suitable number.', 'gscoach' ) . '</p>',
			) );
		}
	}
	...
	private function get_taxonomies( $args = array() ) {
		...
		return (array) apply_filters( 'gs_coach_get_taxonomies', $taxonomies, $r, $args );
	}
	...
	public function add_column_header( $columns = array() ) {
		$columns['order'] = esc_html__( 'Order', 'gscoach' );
	...
	public function set_term_order( $term_id = 0, $taxonomy = '', $order = 0, $clean_cache = false ) {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'gscoach_term_order_set_term_order', $term_id, $taxonomy, $order );
	}
	...
	public function term_order_add_form_field() {
		 ...
		 * @param array $classes
		 */
		$classes = (array) apply_filters( 'gscoach_term_order_add_form_field_classes', $classes, $this );

		?>

		<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
			<label for="order">
				<?php esc_html_e( 'Order', 'gscoach' ); ?>
			</label>
			<input type="number" pattern="[0-9.]+" name="order" id="order" value="0" size="11">
			<p class="description">
				<?php esc_html_e( 'Set a specific order by entering a number (1 for first, etc.) in this field.', 'gscoach' ); ?>
			</p>
		</div>
	...
	public function term_order_edit_form_field( $term = false ) {
		 ...
		 * @param array $classes
		 */
		$classes = (array) apply_filters( 'gscoach_term_order_edit_form_field_classes', $classes, $this );
		...
		<tr class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
			...
			<td>
				...
				<p class="description">
					<?php esc_html_e( 'Terms are usually ordered alphabetically, but you can choose your own order by entering a number (1 for first, etc.) in this field.', 'gscoach' ); ?>
				</p>
			</td>
		</tr>
	...
	public function quick_edit_term_order( $column_name = '', $screen = '', $name = '' ) {
		 ...
		 * @param array $classes
		 */
		$classes = (array) apply_filters( 'gscoach_term_order_quick_edit_field_classes', $classes, $this );
		...
		<fieldset>
			<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
				<label>
					<span class="title"><?php esc_html_e( 'Order', 'gscoach' ); ?></span>
...
add_action( 'init', 'GSCOACH\_term_order', 99 );
