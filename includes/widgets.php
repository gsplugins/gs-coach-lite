...
class Widgets extends \WP_Widget {
	...
	public function get_default_instance() {
		...
		return [
			'title' => __( 'Meet Our Coach', 'gscoach' ),
			'total_mem' => 3,
			'group_mem' => ''
		];
	...
	public function form( $instance ) {
        ...
        $gscoach_taxonomy = 'gs_coach_group';
		$gscoach_term =  get_terms($gscoach_taxonomy); 
		...
		<p> 
			<label for="<?php echo esc_attr( $this->get_field_id('group_mem') ); ?>"><?php echo _e('Coach Group : ') ?></label>
			<select value="<?php echo esc_attr( $group_mem ); ?>" class="widefat" name="<?php echo esc_attr( $this->get_field_name('group_mem') ); ?>" id="<?php echo esc_attr( $this->get_field_id('group_mem') ); ?>">
				<option <?php echo ($group_mem == 'all') ? 'selected' : ''; ?> value=""><?php _e( 'All Groups', 'gscoach' ); ?></option>
				<?php foreach( $gscoach_term as $term) : ?>
	...
	public function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget );
			...
			echo do_shortcode( sprintf( '[gs_coach_sidebar total_mem="%d" group_mem="%s"]', esc_attr($total_mem), esc_attr($group_mem) ) );
...
function register_GSCOACH_widget() {
    register_widget( 'GSCOACH\Widgets' );
}
add_action( 'widgets_init', 'GSCOACH\register_GSCOACH_widget' );