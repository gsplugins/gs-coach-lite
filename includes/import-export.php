...
class Import_Export {
    ...
    public function __construct() {
        add_action('wp_ajax_gscoach_export_data', array($this, 'gscoach_export_data'));
        add_action('wp_ajax_gscoach_import_data', array($this, 'gscoach_import_data'));
        add_action('gs_after_shortcode_submenu', array($this, 'register_sub_menu'));
    }

    public function gscoach_export_data() {
    ...
    public function save_imported_file() {
        ...
        if (move_uploaded_file($file_tmp_path, $dest_file_path)) {
            ...
            if ($zip->open($dest_file_path) === true) {
                ...
                wp_send_json_error(__('File upload failed', 'gscoach'), 400);
            }
        } else {
            wp_send_json_error(__('File upload failed', 'gscoach'), 400);
        }
    ...
    public function gscoach_import_data() {

        // Check for valid nonce
        check_ajax_referer('_gscoach_admin_nonce_gs_');

        // Check for required data
        if (empty($_FILES['import_file'])) wp_send_json_error(__('No import file provided', 'gscoach'), 400);
        ...
        if ( !file_exists($json_import_file) ) wp_send_json_error(__('Invalid file', 'gscoach'), 400);
    ...
    public function delete__coach_data() {
        ...
        // Delete All the Attachments where post_type is 'attachment' && meta_key is '_gs_coach_import_id'
        $attachments = get_posts([
            'posts_per_page'    => -1,
            'post_type'         => 'attachment',
            'meta_key'          => '_gs_coach_import_id'
        ]);
    ...
    public function import__terms($terms) {
        ...
        foreach ($terms as $term) {
            ...
            add_term_meta($inserted_term['term_id'], '_gs_coach_import_id', $term['term_id']);
        ...
        foreach ($attachments as $attachment) {
            ...
            $attachment_data = array(
                ...
                'meta_input'     => array(
                    ...
                    '_gs_coach_import_id' => $attachment['ID']
                )
            );
    ...
    public function import__posts($posts) {
        ...
        foreach ($posts as $post) {
            ...
            $meta_input['_gs_coach_import_id'] = $post['ID'];
    ...
    public function get_imported_post_id($post_id) {
        ...
        $post_id = $wpdb->get_var($wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_gs_coach_import_id' AND meta_value = %d LIMIT 1", $post_id));
    ...
    public function get_imported_term_id($term_id) {
        ...
        $term_id = $wpdb->get_var($wpdb->prepare("SELECT term_id FROM $wpdb->termmeta WHERE meta_key = '_gs_coach_import_id' AND meta_value = %d LIMIT 1", $term_id));
    ...
    public function export__coaches($json_data = []) {
        ...
        foreach ($posts as $post) {
            ...
            unset($post_data['meta_input']['gscoach-demo_data']);
    ...
    public function register_sub_menu() {
        ...
        add_submenu_page(
            ...
            'Import & Export',
            'publish_pages',
            'gs-coach-shortcode#/import-export',
            array($builder, 'view')
        );
    }
    ...
    public function get_taxonomy_list() {
        ...
        $taxonomies = ['gs_coach_group', 'gs_coach_tag', 'gs_coach_language', 'gs_coach_location', 'gs_coach_gender', 'gs_coach_specialty', 'gs_coach_extra_one', 'gs_coach_extra_two', 'gs_coach_extra_three', 'gs_coach_extra_four', 'gs_coach_extra_five'];
    ...
    public function init_zip_file() {
        ...
        // Init the zip file
        $this->zip_file = get_temp_dir() . '/gs-plugins/gs-coach-plugin--export.zip';
